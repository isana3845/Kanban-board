...
def init_db():
    ...
    if not has_column("tasks", "previous_status"): db.execute(
        "ALTER TABLE tasks ADD COLUMN previous_status TEXT DEFAULT ''")
    ...
    if not has_column("boards", "dropzones_enabled"): db.execute(
        "ALTER TABLE boards ADD COLUMN dropzones_enabled INTEGER DEFAULT 1")
    if not has_column("tasks", "start_date"): db.execute("ALTER TABLE tasks ADD COLUMN start_date TEXT DEFAULT ''")
    if not has_column("boards", "progress_done_columns"): db.execute(
        "ALTER TABLE boards ADD COLUMN progress_done_columns TEXT DEFAULT ''")
