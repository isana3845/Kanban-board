...
function openGanttViewer() {
    // Используем активную переменную из state.js
    ...
    // Скрываем главное меню аналитики
    const mainMenu = document.querySelector('#view-analytics .folders-grid');
    if (mainMenu) mainMenu.style.display = 'none';

    // Скрываем другие подвью (логи, архив, бэклог)
    ['logs', 'archive', 'backlog'].forEach(type => {
        const el = document.getElementById(`analytics-${type}-view`);
        if (el) el.style.display = 'none';
    });

    // Показываем view Ганта
    ...
    if (ganttView) {
        ...
        if (window.ganttChart) {
            window.ganttChart.init(boardId);
        } else if (window.GanttChart) {
            // Если класс еще не инстанциирован
...
function closeGanttViewer() {
    ...
    // Возвращаем сетку папок аналитики
    const mainMenu = document.querySelector('#view-analytics .folders-grid');
