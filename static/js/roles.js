...
function applyRoleRestrictions() {
    ...
    // ИСПРАВЛЕНИЕ ЗДЕСЬ: Используем Array.from для преобразования NodeList в массив перед .map
    const dropzones = Array.from(document.querySelectorAll(SELECTORS.cardsDropzones));
    
    [columnSortable, ...dropzones.map(el => el.sortableInstance)]
