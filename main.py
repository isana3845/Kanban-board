...
def init_db():
    db = get_db()
    db.execute(
        """CREATE TABLE IF NOT EXISTS boards (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, owner_username TEXT, wip_enabled INTEGER, wip_todo INTEGER, wip_in_progress INTEGER, wip_done INTEGER)""")
    db.execute("""CREATE TABLE IF NOT EXISTS board_members (board_id INTEGER, username TEXT)""")
    db.execute(
        """CREATE TABLE IF NOT EXISTS tasks (id INTEGER PRIMARY KEY AUTOINCREMENT, board_id INTEGER, title TEXT, assignee TEXT, date TEXT, priority TEXT, description TEXT, status TEXT, creator TEXT, created_at TEXT)""")
    db.execute("""CREATE TABLE IF NOT EXISTS users (username TEXT PRIMARY KEY)""")
    db.execute(
        """CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY AUTOINCREMENT, board_id INTEGER, username TEXT, content TEXT, linked_task_id INTEGER, linked_task_title TEXT)""")
    db.execute(
        """CREATE TABLE IF NOT EXISTS action_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, board_id INTEGER, username TEXT, action_desc TEXT, created_at TEXT)""")
    db.execute(
        """CREATE TABLE IF NOT EXISTS task_comments (id INTEGER PRIMARY KEY AUTOINCREMENT, task_id INTEGER, username TEXT, content TEXT, created_at TEXT)""")
    ...
    if not has_column("tasks", "backlog"): 
        db.execute("ALTER TABLE tasks ADD COLUMN backlog INTEGER DEFAULT 0")
    if not has_column("tasks", "sort_order"): 
        db.execute("ALTER TABLE tasks ADD COLUMN sort_order INTEGER DEFAULT 0")
    if not has_column("tasks", "checkpoints"): 
        db.execute("ALTER TABLE tasks ADD COLUMN checkpoints TEXT DEFAULT '[]'")
    if not has_column("boards", "dropzones_enabled"): 
        db.execute("ALTER TABLE boards ADD COLUMN dropzones_enabled INTEGER DEFAULT 1")
    if not has_column("tasks", "start_date"): 
        db.execute("ALTER TABLE tasks ADD COLUMN start_date TEXT DEFAULT ''")
    if not has_column("boards", "progress_done_columns"): 
...
#диаграмма ганта
# В server.py добавьте этот эндпоинт перед app.mount
@app.get("/api/boards/{board_id}/gantt-data")
def get_gantt_data(board_id: int, session: Optional[str] = Cookie(None)):
    db = get_db()
    # Проверяем доступ
    ...
    # Получаем настройки колонок, которые пользователь выбрал для отображения
    # Предположим, мы передаем список ID колонок через query param, или берем все активные
    # Для простоты возьмем все неархивные задачи с заполненными датами
    tasks = db.execute(
        """SELECT * FROM tasks 
           WHERE board_id=? AND archived=0 AND backlog=0 
           AND start_date != '' AND date != '' AND assignee != ''
...
def get_me(session: Optional[str] = Cookie(None)):
    ...
    db = get_db()
    user = db.execute("SELECT role FROM users WHERE username=?", (session,)).fetchone()
    role = user["role"] if user and user["role"] else "student"
    return {"user_id": session, "username": session, "role": role}

@app.get("/api/auth/role")
def get_role(session: Optional[str] = Cookie(None)):
    """Получить роль текущего пользователя"""
    if not session: raise HTTPException(status_code=401)
    db = get_db()
    user = db.execute("SELECT role FROM users WHERE username=?", (session,)).fetchone()
    if not user: raise HTTPException(status_code=404, detail="User not found")
    return {"username": session, "role": user["role"] or "student"}

@app.put("/api/auth/role")
def set_role(data: dict, session: Optional[str] = Cookie(None)):
    """Установить роль текущего пользователя (student | mentor)"""
    if not session: raise HTTPException(status_code=401)
    role = data.get("role", "student")
    if role not in ("student", "mentor"):
        raise HTTPException(status_code=400, detail="Role must be 'student' or 'mentor'")
    db = get_db()
    db.execute("UPDATE users SET role=? WHERE username=?", (role, session))
    db.commit()
    return {"username": session, "role": role}
...
def create_board(data: BoardData, session: Optional[str] = Cookie(None)):
    ...
    cur = db.execute(
        "INSERT INTO boards (title, owner_username, wip_enabled, wip_todo, wip_in_progress, wip_done, columns_data) VALUES (?, ?, 0, 0, 0, 0, ?)",
...
async def update_board_settings(board_id: int, data: BoardSettingsUpdate, session: Optional[str] = Cookie(None)):
    db = get_db()
    try:
        if data.progress_done_columns is not None:
            db.execute(
                "UPDATE boards SET wip_enabled=?, dropzones_enabled=?, columns_data=?, progress_done_columns=? WHERE id=? AND owner_username=?",
                (data.wip_enabled, data.dropzones_enabled, data.columns_data, data.progress_done_columns, board_id,
                 session))
        else:
            db.execute(
                "UPDATE boards SET wip_enabled=?, dropzones_enabled=?, columns_data=? WHERE id=? AND owner_username=?",
...
def add_member(board_id: int, data: AuthData, session: Optional[str] = Cookie(None)):
    ...
    member_exists = db.execute("SELECT 1 FROM board_members WHERE board_id=? AND username=?",
                               (board_id, data.username)).fetchone()
...
def remove_member(board_id: int, username: str, session: Optional[str] = Cookie(None)):
    ...
    if username == board["owner_username"]: raise HTTPException(status_code=400,
                                                                detail="Нельзя удалить владельца доски")
...
def leave_board(board_id: int, payload: dict = None, session: Optional[str] = Cookie(None)):
    ...
    if board["owner_username"] == session:
        ...
        member = db.execute("SELECT * FROM board_members WHERE board_id=? AND username=?",
                            (board_id, new_owner)).fetchone()
...
async def create_task(data: TaskData, session: Optional[str] = Cookie(None)):
    ...
    max_order = db.execute(
        "SELECT COALESCE(MAX(sort_order), -1) FROM tasks WHERE board_id=? AND status=? AND archived=0 AND backlog=0",
        (data.board_id, data.status)).fetchone()[0]
    ...
    cur = db.execute(
        ...
        (data.board_id, data.title, data.assignee, data.date, data.start_date, data.priority, data.description,
         data.status, data.backlog, data.checkpoints, session, datetime.now().strftime("%Y-%m-%d %H:%M"), sort_order)
    )
...
async def update_task(task_id: int, data: TaskData, session: Optional[str] = Cookie(None)):
    ...
    db.execute(
        ...
        (data.title, data.assignee, data.date, data.start_date, data.priority, data.description, data.status,
    ...
    try:
        while True:
            ...
            if parsed.get("type") == "chat":
                ...
                db = get_db()
                db.execute(
                    "INSERT INTO messages (board_id, username, content, linked_task_id, linked_task_title) VALUES (?, ?, ?, ?, ?)",
...
def get_task_comments(task_id: int, session: Optional[str] = Cookie(None)):
    ...
    access = db.execute("SELECT 1 FROM board_members WHERE board_id=? AND username=?",
                        (task['board_id'], session)).fetchone()
    if not access:
        raise HTTPException(status_code=403, detail="Доступ запрещен")
...
def get_backlog(board_id: int):
    db = get_db()
    tasks = db.execute("SELECT * FROM tasks WHERE board_id=? AND backlog=1 AND archived=0 ORDER BY id DESC",
                       (board_id,)).fetchall()
    return [dict(row) for row in tasks]
...
def search_board(board_id: int, q: str, session: Optional[str] = Cookie(None)):
    ...
    tasks = db.execute(
        "SELECT id, title, backlog, archived FROM tasks WHERE board_id=? AND (title LIKE ? OR description LIKE ?)",
        (board_id, query, query)).fetchall()
    messages = db.execute(
        "SELECT id, username, content FROM messages WHERE board_id=? AND content LIKE ? ORDER BY id DESC LIMIT 20",
        (board_id, query)).fetchall()
    logs = db.execute(
        "SELECT id, username, action_desc as content FROM action_logs WHERE board_id=? AND action_desc LIKE ? ORDER BY id DESC LIMIT 20",
        (board_id, query)).fetchall()
    ...
    for t in tasks:
        ...
        if t["backlog"] == 1:
            results["backlog"].append(task_data)
        elif t["archived"] == 1:
            results["archive"].append(task_data)
        else:
            results["board"].append(task_data)
